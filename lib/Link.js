...
const DEFAULT_PLATFORM = '_',
  LINKS_DIR = path.resolve(__dirname, '../data/'),
  LINKS_PATH = path.resolve(LINKS_DIR, 'links.json'),
...
class Link {
   ...
   * @param {number} [keyLength] - The character length of the new key.
   * @returns {(string|boolean)}
   */
  store(url, keyLength = 5) {
    if (!this.links) {
      return false;
    }
